...
class SEGAN():
    ...
    def __init__(self, sess, type_model='original', retrain=False, model_path='save_model',):
        ...
        if type_model == 'original':
            self.generator = Unet(self.list_filter)
            self.discriminator = Discriminator(self.list_filter)
        elif type_model == 'UNetGan':
            self.generator = UNet_GAN(input=self.Z, num_block=cf.num_block_g, dil_block=cf.num_dila_block,
                                      rate_list=cf.list_rate, basic_channel=cf.basic_channel_g, advance_gan=False)
            self.discriminator = DUnetGan(num_block=cf.num_block_d, basic_channel=cf.basic_channel_d, list_rate=cf.list_rate)
    ...
    def build(self):
        ...
        self.opt_d = tf.train.AdamOptimizer(self.lr).minimize(self.loss_d, var_list=D_var_list)
        self.opt_g = tf.train.AdamOptimizer(self.lr).minimize(self.loss_total_g, var_list=G_var_list)
    ...
    def load(self, saved_path):
        '''if saved_path is not os.path.exists(sav_path):
    ...
    def train(self, save_model, load_model=None):
        ...
        for epoch in range(self.num_epoches):
            for iter in range(self.num_iters):
                ...
                __, loss_g_, loss_only_g, loss_l1 = self.sess.run([self.opt_g, self.loss_total_g, self.loss_g, self.loss_g_l1], feed_dict={self.X: wav, self.Z:noise, self.lamda:lamda})
